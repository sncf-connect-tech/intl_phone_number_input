...
/// A [TextFormField] for [IntlPhoneNumberInput].
///
...
class IntlPhoneNumberInput extends StatefulWidget {
  ...
  final InputBuilder? textInputBuilder;
  final CountriesFilterPredicate? countriesFilterPredicate;

  /// A [String] representing an alpha 2 code
  /// represents the country that should always on top of the list
  final String? firstCountryInList;
  ...
  IntlPhoneNumberInput({
    ...
    this.countriesFilterPredicate,
    this.firstCountryInList,
  }) : super(key: key);

  @override
  State<StatefulWidget> createState() => IntlPhoneNumberInputState();
}

class IntlPhoneNumberInputState extends State<IntlPhoneNumberInput> {
  ...
  @override
  void didUpdateWidget(IntlPhoneNumberInput oldWidget) {
  ...
  void loadCountries({Country? previouslySelectedCountry}) {
    if (this.mounted) {
      ...
      final CountryComparator countryComparator = widget.selectorConfig.countryComparator ?? (a, b) => 0;
      final CountriesFilterPredicate filterPredicate = widget.countriesFilterPredicate ?? (country) => true;

      countries = countries.where(filterPredicate).sorted(countryComparator);
      if (widget.firstCountryInList != null) {
        final firstCountry = countries.firstWhereOrNull((element) {
          return element.alpha2Code?.toLowerCase() == widget.firstCountryInList?.toLowerCase();
        });
        if (firstCountry != null) {
          countries..remove(firstCountry)..insert(0, firstCountry);
        }
      }
      countries = countries.toSet().toList(growable: false);
...
class _InputWidgetView extends WidgetView<IntlPhoneNumberInput, IntlPhoneNumberInputState> {
  final IntlPhoneNumberInputState state;
    ...
    return Container(
      child: Row(
        ...
        children: <Widget>[
          ...
          Flexible(
            child: widget.textInputBuilder?.call(context, state.countries) ??
                TextFormField(
